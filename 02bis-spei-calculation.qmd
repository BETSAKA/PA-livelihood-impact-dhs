...
ggplot(spei_tbl, aes(x = year, y = spei_mean)) + 
  ...
  labs(
    title = "Annual variation of the SPEI index for one cluster in the period 1980-2021",
    x = "Year",
    y = "SPEI mean"
  ) + 
  theme_minimal() + 
...
ggplot(spei_df, aes(x = year, y = spei_mean, group = DHSCLUST)) + 
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") + 
  labs(
    title = "Annual SPEI index by cluster in period 1980-2021",
    x = "Year",
    y = "SPEI mean"
  ) + 
  theme_minimal() + 
