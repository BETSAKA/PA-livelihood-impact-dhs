...
strict_effect <- tidy_es %>%
  ...
  dplyr::select(
    year, 
    estimate_strict = estimate, 
    conf.low_strict = conf.low,
    conf.high_strict = conf.high
  )
...
usage_effect <- left_join(strict_effect, usage_diff, by = "year") %>%
  mutate(
    estimate = estimate_strict + estimate_diff,
    conf.low = conf.low_strict + conf.low_diff,
    conf.high = conf.high_strict + conf.high_diff,
