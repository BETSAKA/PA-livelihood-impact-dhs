...
out <- map_dfr(years, function(y) {
  ...
  tibble(
    ...
    `Total Number of Households`        = Raw_HH_all,
    `Rural Sub-sample`                  = Raw_HH_rural,
    `Excluded Households`               = Excluded_group,
    `Treatment vs Control Group`        = Treatment_Control,
    `Households After Removing Missing Values` = After_NA_drop,
    `Treated Households`                = Treated,
    `Matched Observations`              = Matched_rows
  )
})
