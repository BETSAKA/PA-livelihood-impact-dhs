...
# Préparation de la classification des clusters
cluster_classification <- read_csv(
  "data/derived/cluster_treatment_classification_staggered.csv",
...
# Fonction de préparation des données ménages 
make_desc_wealth <- function(year){
  ...
  readRDS(paste0("data/derived/hh_", year, "_rural_simpler.rds")) %>%
    ...
    left_join(
      ...
      by = c("DHSCLUST", "DHSYEAR")
    ) %>%
    mutate(
      GROUP = replace_na(GROUP, "Excluded")
    )
}
...
kable(
  desc_all,
  caption = "Statistiques descriptives du wealth index rural (centile et z-score) par groupe et par année",
  digits = 2
)
...
ggplot(
  ...
  scale_fill_manual(
    name = "Groupe",
    values = c(
      "Control"   = scales::alpha("blue", 0.6),
      "Treatment" = scales::alpha("red",  0.6),
      "Excluded"  = scales::alpha("grey", 0.6)
    )
  ) +
  labs(
    title = "Distribution of the rural wealth index (percentile)",
    subtitle = "Comparison by group and DHS year",
    x = "Group",
...
# Boxplots - Zscore du Wealth index en centile
ggplot(
  wealth_plot_data %>%
  ...
  labs(
    title = "Distribution of the wealth index zscore",
    subtitle = "Comparison by group et by DHS year",
    x = "Group",
    y = "wealth index zscore"
  ) +
  theme_minimal(base_size = 13) +
  theme(
    legend.position = "right",
