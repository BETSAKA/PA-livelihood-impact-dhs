...
Le quantile treatment effect mesure l'effet du traitement à différents points de la distribution de la variable de résultat, le zscore du wealth index. Il montre comment le traitement affecte les ménages les plus pauvres (quantiles bas), la classe moyenne et les ménages les plus aisés (quantiles élevés).
...
L'estimation du DID statique montre qu'être situé dans un rayon de 10 km autour d'une aire protégée (que ce soit pour les aires strictes ou pour les aires à usage multiple) n'a pas d'effet significatif sur le centile de richesse.

L'analyse en event study confirme également dans l'ensemble l'absence d'effet significatif avant le traitement. L'hypothèse de tendances parallèles est donc respectée. Toutefois, deux après le traitement (rel_year_binned = 2), les ménages proches d'une aire protégée à usage multiple connaissent également une diminution significative de leur centile de richesse (coefficient = -10.96\*).

[**Outcome:**]{.underline} zscore_wealth

L'estimation DID statique indique que les ménages dans la zone traitée (cluster situé dans un rayon de 10 km) voient leur richesse augmenter légèrement après la création des aires protégées(treat_on = 0.02585\*). Toutefois, les différences selon le statut IUCN ne sont pas significatives dans le modèle. En analysant l'étude par event study, le résultat montre qu'il n'y a aucun effet significatif entre les groupes avant la création des aires protégées. Mais au bout de 2 ans après la création des aires protégées, les aires protégées strictes montrent un effet positif significatif sur le score standardisé de l'indice de richesse des ménages situés dans un rayon de 10 km. Les aires protégées à usage multiple montrent également un effet positif, mais non significatif.
