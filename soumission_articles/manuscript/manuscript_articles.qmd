...
The remainder of the paper proceeds as follows [@theoryofchange], presents the conceptual framework and hypotheses. [@data] describes the data sources. [@descriptivestatistics] outlines the identification strategy and econometric methods. [@studydesign] presents the econometric approaches used to assess the effect of PAs on household livelihoods, the effect of PAs on inequalities between households, and the heterogeneity of effects according to the type of PAs governance. [@resultats] presents our main results and proposes a series of robustness tests. [@discussionandconclusion] concludes.
...
### Data {#data}
This study assesses the impact or the creation of PAs on rural household well-being between 2008 and 2021, based on geolocated data on PAs (Word Database on Protected Areas (WDPA)), environmental characteristics (downloaded from the *mapme.biodiversity* package), and the socioeconomic conditions of households (Demographic and Health Surveys (DHS)). Household living standards are the primary outcome, with inequality as a secondary outcome, and demographic and environmental varaibles are included as controls to improve the precision of treatment effect estimates. We use three DHS surveys (1997, 2008, 2021), taking 2008 as the reference year. The treated group comprises rural households located within 10 km of PAs established between 2008 and 2021 (in red), while the control group includes rural households located more than 10 km away with comparable characteristics (in blue). Rural households near PAs created before 2008 and households in urban areas are excluded.  
A comprehensive overview of the data set along with the selected variables is outlined within the pre-analysis plan.
...
$Treatment_i$: Binary variable that takes the value of 1 for households $i$ affected by a PAs (treatment households) and 0 for households $i$ not affected by PAs (control households)
...
$δ$: Coefficient of interest representing the effect of PAs creation on household living standards $i$ in year $t$
