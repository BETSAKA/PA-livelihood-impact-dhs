...
To assess the impact of PA surrounding the well-being of households, it is essentials to identify the counterfactual scenario, that is, what would have occurred in the absence of protection. We therefore applied genetic matching combined with Mahalanobis distance matching and a caliper of 0.25 to reduce observable differences between treated and control units and to quantify the similarly between matched observations. This matching procedure enhances the robustness and credibility of the empirical results (Appendix). The underlying assumption is that, after matching, treated and control units would exhibit identical expected outcomes in the absence of PAs. Post-matching, we employed a difference-in-difference (DID) approach to estimate the causal effect associated with the PAs creation. This method consists in comparing the wealth index of households in treated and control groups before and after the PA creation(Appendix). The DID strategy relies on the parallel tyrends assumption, which posits that treated and untreated areas followed similar trajectories prior to the intervention.
...
#| tbl-cap: "Standardized Mean Difference of the Year 2008 "
#| warning: false
#| message: false
#| echo: false

balance_long_2008 <- read.csv("table/balance_long_2008.csv")
balance_long_2008
```
