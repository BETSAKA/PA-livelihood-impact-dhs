...
dynamic_wdpa_vect <- vect("data/PA_creation/dynamic_wdpa(2).gpkg") %>%
...
```{r fig.cap="Figure 3: Protected Areas in Madagascar"}

### Take last boundary
dynamic_pa_last <- dynamic_wdpa_vect %>%
  dplyr::filter(zone_type == "external_boundary") %>%
  select(WDPAID) %>%
  group_by(WDPAID) %>%
  slice_tail(n = 1) %>%
...
dynamic_pa_intersec <- pblapply(
  unique(dynamic_wdpa_vect$WDPAID), 
  function(id) {
    sub <- dynamic_wdpa_vect %>%
...
core_pa_last <- dynamic_wdpa_vect %>%
...
```{r fig.cap="Figure 4: Protected Areas in Madagascar"}

# Convertir en sf (dynamic_pa_last est un SpatVector terra)
pa_sf <- sf::st_as_sf(dynamic_pa_last) %>%
  left_join(dynamic_wdpa_metadata, by = "WDPAID") %>% 
  st_make_valid()

# Export the cleaned data 
sf::st_write(pa_sf, "data/derived/pa_sf.csv", delete_dsn = TRUE)
...
tm_shape(wdpa_from_2008) +
  ...
  tm_add_legend(
    ...
    labels = c("Before 2008", "From 2008")
  ) +
...
# Liste détaillée des PA créées entre 2008 et 2021

start_year <- 2008
end_year   <- 2021

 pa_list_2008_2021 <- pa_list_since2008 %>%
  filter(
    STATUS_YR >= start_year,
    STATUS_YR <= end_year
  ) %>%
  arrange(STATUS_YR)

# Créer le tableau gt
gt_pa_list_2008_2021 <- pa_list_2008_2021 %>%
  gt() %>%
  tab_header(
    title = md("**Protected Areas Created 2008–2021**"),
    subtitle = md(
      paste0(
        "*Madagascar — Sorted by year of creation — Period: ",
        start_year, "–", end_year, "*"
      )
    )
  ) %>%
  ...
  cols_align(
    align = "left",
    columns = c(NAME, IUCN_CAT, STATUS, MANAGER)
  ) %>%
  cols_align(
    align = "right",
    columns = c(STATUS_YR, PA_AREA_KM2)
  ) %>%
  tab_style(
    style = cell_fill(color = "#e8f5e9"),
    locations = cells_body(
      rows = seq(2, nrow(pa_list_2008_2021), 2)
    )
  ) %>%
  tab_style(
    style = list(
  ...
  tab_source_note(
    md("**Source:** World Database on Protected Areas (WDPA), 2025.")
  ) %>%
  tab_options(
    table.font.names = "Times New Roman",
    table.font.size = px(13),
...
# Sauvegarde
write_rds(gt_pa_list_2008_2021, "paper/tables/tbl_pa_list_2008_2021.rds")
gtsave(gt_pa_list_2008_2021, "paper/tables/tbl_pa_list_2008_2021.png", expand = 10, zoom = 3)


write_csv(gt_pa_list_2008_2021[["_data"]], "paper/tables/pa_list_2008_2021.csv")

gt_pa_list_2008_2021


# Tableau récapitulatif des aires protégées créées entre 1997 et 2021
# Definition de la fonction
library(gt)
...
# Tableau — Comparaison 1997-2008 / 2008-2021
summary_table <- pa_sf %>%
  st_drop_geometry() %>%
  filter(STATUS_YR >= period_start, STATUS_YR <= end_year) %>%
  mutate(
    Creation_Period = if_else(
      STATUS_YR < pivot_year,
      paste0(period_start, "–", pivot_year),
      paste0(pivot_year, "–", end_year)
    ),
    Creation_Period = factor(
      Creation_Period,
      levels = c(paste0(period_start, "–", pivot_year),
                 paste0(pivot_year, "–", end_year))
    )
  ) %>%
  group_by(Creation_Period, .drop = FALSE) %>%
  summarise(
    Count    = n(),
    Area_km2 = sum(PA_AREA_KM2, na.rm = TRUE),
    .groups  = "drop"
...
gt_table <- summary_table %>%
  ...
  tab_header(
    title    = md("**Protected Areas by Period of Creation**"),
    subtitle = md(paste0("*Madagascar — Periods: ", period_start, "–", pivot_year,
                         " and ", pivot_year, "–", end_year,
                         " — Terrestrial areas only*"))
  ) %>%
  cols_label(
    Creation_Period = md("**Creation Period**"),
...
write_rds(gt_table, "paper/tables/tbl-crea_1997_2008.rds")
gtsave(gt_table, "paper/tables/tbl-crea_1997_2008.png", expand = 10, zoom = 3)
gt_table
...
Sur la période de 2008-2021, 54 aires protégées ont été créées.
...
```{r fig.cap="Figure 6: DHS survey clusters relative to existing protected areas"}
...
ma_carte <- tm_shape(contour_mada) +
  tm_borders(col = "black", lwd = 1) +
  ...
  tm_shape(wdpa_from_2008) +
  tm_polygons(fill = "green", fill_alpha = 0.5, col = "black") +

  tm_shape(wdpa_before_2008) +
  tm_polygons(fill = "darkgreen", fill_alpha = 0.5, col = "black") +
