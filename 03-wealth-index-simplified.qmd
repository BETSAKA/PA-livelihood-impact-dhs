...
We first check whether the centile classification of households is consistent between the national wealth score (`hv270`) and the rural-specific wealth score (`hv270a`). The comparison is done for rural households in the 2021 DHS. Both indices are transformed into centiles, and we examine their agreement through simple proportions, correlation, and a scatterplot.
...
hh21_rur <- hh21 %>%
  ...
  transmute(
    hv001, hv002, hv005,
    c_hv270  = ntile(hv270,  100),
    c_hv270a = ntile(hv270a, 100)
  )

# Centile agreement
mean(hh21_rur$c_hv270 == hh21_rur$c_hv270a, na.rm = TRUE)

cor.test(hh21_rur$c_hv270, hh21_rur$c_hv270a,
         method = "spearman", use = "complete.obs")


#  Scatterplot with both axes
ggplot(hh21_rur, aes(x = c_hv270, y = c_hv270a)) +
  geom_point(alpha = 0.3) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 0.3, color = "blue") +
  labs(
    title = "Rural households (2021): hv270 vs hv270a centiles",
    x = "Centiles from hv270 (national wealth score)",
    y = "Centiles from hv270a (rural wealth score)"
  )
...
For comparability over time, we rely directly on the DHS wealth score (`hv270`) available in each household dataset. In the 1997 DHS, the wealth index is stored separately and must be merged by household identifier. For subsequent surveys (2008–2021), it is included directly in the household file.
...
hh08 <- read_dta("data/raw/dhs/DHS_2008/MDHR51DT/MDHR51FL.DTA") %>%
  select(hv001, hv002, hv005, hv025, hv270)
  ...
  df_rur %>%
    mutate(
      wealth_centile_rural_weighted = 
        cut(hv270, breaks = c(-Inf, thr),
            labels = 1:100, include.lowest = TRUE, right = TRUE) %>% 
        as.integer(),
      wealth_centile_rural_simple = ntile(hv270, 100)
    ) %>%
    select(-.w)
}
