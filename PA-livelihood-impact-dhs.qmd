...
# groupe : "Traitement" ou "controle"
matching_2008 <- gps_cluster_2008 %>%
  filter(groupe %in% c("Traitement", "Controle")) %>%
  mutate(
    treatment = ifelse(groupe == "Traitement", 1, 0),
    treecover_area_2000 = sapply(treecover_area, extract_value, year = "2000"), 
    slope_val = sapply(slope, extract_value),
    elevation_val = sapply (elevation, extract_value),
    population_count_val = sapply(population_count, extract_value),
    traveltime_val = sapply(traveltime_2000, extract_value)
  ) %>% 
  drop_na(all_of(variables_matching))

X_match <- as.data.frame(st_drop_geometry(matching_2008[, c("treecover_area_2000", "slope_val", "elevation_val", "population_count_val", "traveltime_val")]))
...
matching_result_2008 <- matchit(
  treatment ~ treecover_area_2000 + slope_val + elevation_val + population_count_val + traveltime_val,
  data = matching_2008 %>% filter(groupe != "Exclu"),
  method = "genetic", 
  distance = "mahalanobis",
  gen.match = gen_match_model,
  caliper = c(
    treecover_area_2000 = 0.25, 
    slope_val = 0.25,
    elevation_val = 0.25,
...
X_match <- as.data.frame(st_drop_geometry(matching_2021[, c("treecover_area_2000", "slope_val", "elevation_val", "population_count_val", "traveltime_val")]))
...
Avant le matching, il se trouve que plusieurs variables montrent un déséquilibre entre le groupe traité et le groupe contrôle. Après le matching, l'équilibre entre les groupes pour les covariables est rétabli avec 23 unités traitées appariées avec 23 contrôles.
...
Après le matching, l'équilibre entre les groupes pour les covariables est rétabli avec 42 unités traitées appariées avec 42 contrôles.
