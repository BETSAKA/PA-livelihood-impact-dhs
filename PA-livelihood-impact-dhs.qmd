...
# Fonction pour QQ plot corrigée
plot_qq <- function(var, data, treat_col = "treatment") {
  treated <- data[data[[treat_col]] == 1, var]
  control <- data[data[[treat_col]] == 0, var]
  
  # Transformer en vecteurs numériques si nécessaire
  treated <- unlist(treated)
  control <- unlist(control)
  
  # Tracer le QQ plot
  qqplot(control, treated, 
         main = paste("QQ plot pour", var), 
         xlab = "Quantiles Contrôle", 
         ylab = "Quantiles Traitement")
  abline(0, 1, col = "red") # Ligne de référence
}

# Application pour 2008
plot_qq("treecover_area_val", matching_2008)
plot_qq("slope_val", matching_2008)
plot_qq("elevation_val", matching_2008)
plot_qq("population_count_val", matching_2008)
plot_qq("traveltime_val", matching_2008)

# Application pour 2021
plot_qq("treecover_area_val", matching_2021)
plot_qq("slope_val", matching_2021)
plot_qq("elevation_val", matching_2021)
plot_qq("population_count_val", matching_2021)
plot_qq("traveltime_val", matching_2021)


# Résumé des appariements pour 2008
summary(matching_result_2008)

# Résumé des appariements pour 2021
...
```{r}
# Matching avec caliper de 0.5 SD Mahalanobis 
matching_result_2008 <- matchit(
  treatment ~ treecover_area_val + slope_val + elevation_val + population_count_val + traveltime_val,
  data = matching_2008 %>% filter(groupe != "Exclu"),
  method = "genetic", 
