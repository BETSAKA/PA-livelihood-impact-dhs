...
# Créer une base combinée  des années d'étude
gps_cluster_2008$year <- 2008
gps_cluster_2021$year <- 2021

gps_panel <- bind_rows(gps_cluster_2008, gps_cluster_2021)
...
matching_result <- gps_panel %>%
  ...
  drop_na(treecover_area_val, slope_val, elevation_val, population_count_val, traveltime_val)
 

X_match <- as.data.frame(st_drop_geometry(matching_result[, c("treecover_area_val", "slope_val", "elevation_val", "population_count_val", "traveltime_val")]))


# Matching avec MatchIt
set.seed(123)

gen_match_model <- GenMatch(
  Tr = matching_result$treatment, 
  X = X_match,
...
bal_tab_post <- bal.tab(matchit_result)
...
```{r }
# Matching avec caliper de 0.30 SD Mahalanobis 
# Appariement avec caliper assoupli
matchit_result_cal <- matchit(
  treatment ~ treecover_area_val + slope_val + elevation_val + population_count_val + traveltime_val,
  data = matching_result %>% filter(groupe != "Exclu"),
  method = "genetic", 
  distance = "mahalanobis",
  gen.match = gen_match_model,
  caliper = c(
    treecover_area_val = 0.5,  # 
    slope_val = 0.5,
    elevation_val = 0.5,
    population_count_val = 0.5,
    traveltime_val = 0.5
  )
)

# Vérification de l'équilibre après appariement avec caliper assoupli
bal_tab_post_cal <- bal.tab(matchit_result_cal)
print(bal_tab_post_cal)

# Visualisation des distances de Mahalanobis après l'appariement avec caliper assoupli
hist(matchit_result_cal$distance, main = "Distances de Mahalanobis après appariement avec caliper assoupli", xlab = "Distance")
