...
if (file.exists("data/donnees_geo_menages/2008/gps_2008_wc_spei.rds")) {
  gps_2008_wc_spei <- read_rds("data/donnees_geo_menages/2008/gps_2008_wc_spei.rds")
} else {
 ...
 write_rds(x = gps_2008, file = "Data/donnees_geo_menages/2008/gps_2008_wc_spei.rds")
```

```{r 2021}
# Latitude calculation
if (file.exists("data/donnees_geo_menages/2021/gps_2021_wc_spei.rds")) {
  gps_2021_wc_spei <- read_rds("data/donnees_geo_menages/2021/gps_2021_wc_spei.rds")
} else {
  gps_2021_wc_spei <- gps_2021 %>% 
    mutate(
      lat = st_coordinates (st_centroid(geometry))[,2])
  
  # Extract the first cluster's climate time series
  row1 <- gps_2021_wc_spei [1,]
  ...
  # SPEI sous forme de map
  gps_2021_wc_spei <- gps_2021_wc_spei %>%
    mutate(
      spei_wc = pmap(
        list(tmin = temperature_min_wc,
             tmax = temperature_max_wc,
 ...
 write_rds(x = gps_2021, file = "Data/donnees_geo_menages/2021/gps_2021_wc_spei.rds")
```
...
# Variables de matching : treecover_area, slope, elevation, population_count, travel_time
variables_matching <- c("treecover_area", "slope", "elevation", "population_count", "traveltime_2000")

# Pré-traitement des données
matching_2008 <- gps_cluster_2008 %>%
  filter(groupe %in% c("Traitement", "Controle")) %>%
  mutate(
    treatment = ifelse(groupe == "Traitement", 1, 0),
    treecover_area_val = map_dbl(treecover_area, ~ {
      # Filtrer pour datetime == "2000-01-01"
      value <- .x$value[.x$datetime == "2000-01-01"]
      # Retourner NA si aucune valeur n'est trouvée
      if (length(value) == 0) NA else value
    }),
    slope_val = map_dbl(slope, ~ {
      value <- .x$value[.x$datetime == "2000-02-01"]
      if (length(value) == 0) NA else value
    }),
    elevation_val = map_dbl(elevation, ~ {
      value <- .x$value[.x$datetime == "2000-02-01"]
      if (length(value) == 0) NA else value
    }),
    population_count_val = map_dbl(population_count, ~ {
      value <- .x$value[.x$datetime == "2000-01-01"]
      if (length(value) == 0) NA else value
    }),
    traveltime_val = map_dbl(traveltime_2000, ~ {
      value <- .x$value[.x$datetime == "2000-01-01"]
      if (length(value) == 0) NA else value
    })
  ) %>% 
  drop_na(all_of(variables_matching)) # Supprime les lignes avec des valeurs manquantes

X_match <- as.data.frame(st_drop_geometry(matching_2008[, c("treecover_area_val", "slope_val", "elevation_val", "population_count_val", "traveltime_val")]))
...
matching_result_2008 <- matchit(
  treatment ~ treecover_area_val + slope_val + elevation_val + population_count_val + traveltime_val,
  data = matching_2008 %>% filter(groupe != "Exclu"),
  method = "genetic", 
  distance = "mahalanobis",
  gen.match = gen_match_model,
  caliper = c(
    treecover_area_val = 0.25, 
    slope_val = 0.25,
    elevation_val = 0.25,
    population_count_val = 0.25,
    traveltime_val = 0.25
)
...
# Pré-traitement des données
matching_2021 <- gps_cluster_2021 %>%
  filter(groupe %in% c("Traitement", "Controle")) %>%
  mutate(
    treatment = ifelse(groupe == "Traitement", 1, 0),
    treecover_area_val = map_dbl(treecover_area, ~ {
...
X_match <- as.data.frame(st_drop_geometry(matching_2021[, c("treecover_area_val", "slope_val", "elevation_val", "population_count_val", "traveltime_val")]))
...
matching_result_2021 <- matchit(
  treatment ~ treecover_area_val + slope_val + elevation_val + population_count_val + traveltime_val,
  data = matching_2021 %>% filter(groupe != "Exclu"),
  method = "genetic", 
  distance = "mahalanobis",
  gen.match = gen_match_model,
  caliper = c(
...
matching_2008_df <- st_drop_geometry(matching_2008)


# Vérification de l'équilibre des covariables avant matching
bal_tab_pre <- bal.tab(
  treatment ~ treecover_area_val + slope_val + elevation_val + population_count_val + traveltime_val,
  data = matching_2008_df
)

print(bal_tab_pre)

# Vérification de l'équilibre des covariables après matching
bal_tab_post <- bal.tab(matching_result_2008, quick = FALSE)

print(bal_tab_post)

# Plot de l'équilibre avant matching
love.plot(bal_tab_pre, 
          stats = "mean.diffs", 
          abs = TRUE,
          threshold = 0.1, 
          var.order = "unadjusted", 
          title = "Equilibre des covariables avant Matching (2008)")

# Plot de l'équilibre après matching
love.plot(bal_tab_post,
          stats = "mean.diffs", 
          abs = TRUE,
          threshold = 0.1,
          
          var.order = "unadjusted", 
          title = "Equilibre des covariables après Matching (2008)")

```

Avant le matching, il se trouve que plusieurs variables montrent un déséquilibre entre le groupe traité et le groupe contrôle.

Après le matching, l'équilibre entre les groupes pour les covariables est rétabli avec 23 unités traitées appariées avec 23 contrôles.

```{r 2021}
library(cobalt)
library(sf)

matching_2021_df <- st_drop_geometry(matching_2021)


# Vérification de l'équilibre des covariables avant matching
bal_tab_pre <- bal.tab(
  treatment ~ treecover_area_val + slope_val + elevation_val + population_count_val + traveltime_val,
  data = matching_2021_df
)

print(bal_tab_pre)

# Vérification de l'équilibre des covariables après matching
bal_tab_post <- bal.tab(matching_result_2021, quick = FALSE)
...
love.plot(bal_tab_pre, 
          ...
          threshold = 0.1, 
          var.order = "unadjusted", 
          title = "Equilibre des covariables avant Matching (2021)")
...
love.plot(bal_tab_post,
          ...
          var.order = "unadjusted", 
          title = "Equilibre des covariables après Matching (2021)")


```

Pour 2021, après le matching, l'équilibre entre les groupes pour les covariables est rétabli avec 42 unités traitées appariées avec 42 contrôles.

## Vérification des distributions des covariables

```{r 2008}
# Extraire les données appariées pour 2008
matched_data_2008 <- match.data(matching_result_2008)

# Ajouter une colonne pour indiquer "Avant" ou "Après" le matching
matching_2008$matching_status <- "Avant"
matched_data_2008$matching_status <- "Après"

# Combiner les deux jeux de données
combined_data_2008 <- rbind(
  matching_2008[, c("treecover_area_val", "slope_val", "elevation_val", "population_count_val", "traveltime_val", "treatment", "matching_status")],
  matched_data_2008[, c("treecover_area_val", "slope_val", "elevation_val", "population_count_val", "traveltime_val", "treatment", "matching_status")]
)

# Histogramme pour treecover_area_val avant et après matching
ggplot(combined_data_2008, aes(x = treecover_area_val, fill = as.factor(treatment))) +
  geom_histogram(alpha = 0.6, position = "identity", bins = 30) +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red"), labels = c("Contrôle", "Traitement")) +
  labs(title = "Distribution de treecover_area_val 2008",
       x = "treecover_area_val", y = "Fréquence",
       fill = "Groupe") +
  theme_minimal()

# Densité pour slope_val avant et après matching
ggplot(combined_data_2008, aes(x = slope_val, color = as.factor(treatment), fill = as.factor(treatment))) +
  geom_density(alpha = 0.3) +
  facet_wrap(~matching_status) +
  scale_color_manual(values = c("0" = "blue", "1" = "red"), labels = c("Contrôle", "Traitement")) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red"), labels = c("Contrôle", "Traitement")) +
  labs(title = "Distribution de slope_val 2008",
       x = "slope_val", y = "Densité",
       color = "Groupe", fill = "Groupe") +
  theme_minimal()


# Boxplot pour elevation_val avant et après matching
ggplot(combined_data_2008, aes(x = as.factor(treatment), y = elevation_val, fill = as.factor(treatment))) +
  geom_boxplot() +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red"), labels = c("Contrôle", "Traitement")) +
  labs(title = "Comparaison de elevation_val 2008",
       x = "Groupe", y = "elevation_val",
       fill = "Groupe") +
  theme_minimal()

# Densité pour population_count_val avant et après matching
ggplot(combined_data_2008, aes(x = population_count_val, fill = as.factor(treatment), color = as.factor(treatment))) +
  geom_density(alpha = 0.3) +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red")) +
  scale_color_manual(values = c("0" = "blue", "1" = "red")) +
  labs(title = "Densité de population_count_val 2008", x = "population_count_val", y = "Densité", fill = "Groupe", color = "Groupe") +
  theme_minimal()

# Boxplot
ggplot(combined_data_2008, aes(x = as.factor(treatment), y = population_count_val, fill = as.factor(treatment))) +
  geom_boxplot() +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red")) +
  labs(title = "Comparaison de population_count_val 2008", x = "Groupe", y = "population_count_val", fill = "Groupe") +
  theme_minimal()

# Boxplot pour traveltime_val avant et après matching
# Densité
ggplot(combined_data_2008, aes(x = traveltime_val, fill = as.factor(treatment), color = as.factor(treatment))) +
  geom_density(alpha = 0.3) +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red")) +
  scale_color_manual(values = c("0" = "blue", "1" = "red")) +
  labs(title = "Densité de traveltime_val 2008", x = "traveltime_val", y = "Densité", fill = "Groupe", color = "Groupe") +
  theme_minimal()

# Boxplot
ggplot(combined_data_2008, aes(x = as.factor(treatment), y = traveltime_val, fill = as.factor(treatment))) +
  geom_boxplot() +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red")) +
  labs(title = "Comparaison de traveltime_val 2008", x = "Groupe", y = "traveltime_val", fill = "Groupe") +
  theme_minimal()

```

```{r 2021}
# Extraire les données appariées pour 2021
matched_data_2021 <- match.data(matching_result_2021)

# Ajouter une colonne pour indiquer "Avant" ou "Après" le matching
matching_2021$matching_status <- "Avant"
matched_data_2021$matching_status <- "Après"

# Combiner les deux jeux de données
combined_data_2021 <- rbind(
  matching_2021[, c("treecover_area_val", "slope_val", "elevation_val", "population_count_val", "traveltime_val", "treatment", "matching_status")],
  matched_data_2021[, c("treecover_area_val", "slope_val", "elevation_val", "population_count_val", "traveltime_val", "treatment", "matching_status")]
)

# Histogramme pour treecover_area_val avant et après matching
ggplot(combined_data_2021, aes(x = treecover_area_val, fill = as.factor(treatment))) +
  geom_histogram(alpha = 0.6, position = "identity", bins = 30) +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red"), labels = c("Contrôle", "Traitement")) +
  labs(title = "Distribution de treecover_area_val 2021",
       x = "treecover_area_val", y = "Fréquence",
       fill = "Groupe") +
  theme_minimal()

# Densité pour slope_val avant et après matching
ggplot(combined_data_2021, aes(x = slope_val, color = as.factor(treatment), fill = as.factor(treatment))) +
  geom_density(alpha = 0.3) +
  facet_wrap(~matching_status) +
  scale_color_manual(values = c("0" = "blue", "1" = "red"), labels = c("Contrôle", "Traitement")) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red"), labels = c("Contrôle", "Traitement")) +
  labs(title = "Distribution de slope_val 2021",
       x = "slope_val", y = "Densité",
       color = "Groupe", fill = "Groupe") +
  theme_minimal()
...
# Boxplot pour elevation_val avant et après matching
ggplot(combined_data_2021, aes(x = as.factor(treatment), y = elevation_val, fill = as.factor(treatment))) +
  geom_boxplot() +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red"), labels = c("Contrôle", "Traitement")) +
  labs(title = "Comparaison de elevation_val 2021",
       x = "Groupe", y = "elevation_val",
       fill = "Groupe") +
  theme_minimal()

# Densité pour population_count_val avant et après matching
ggplot(combined_data_2021, aes(x = population_count_val, fill = as.factor(treatment), color = as.factor(treatment))) +
  geom_density(alpha = 0.3) +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red")) +
  scale_color_manual(values = c("0" = "blue", "1" = "red")) +
  labs(title = "Densité de population_count_val 2021", x = "population_count_val", y = "Densité", fill = "Groupe", color = "Groupe") +
  theme_minimal()

# Boxplot
ggplot(combined_data_2021, aes(x = as.factor(treatment), y = population_count_val, fill = as.factor(treatment))) +
  geom_boxplot() +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red")) +
  labs(title = "Comparaison de population_count_val 2021", x = "Groupe", y = "population_count_val", fill = "Groupe") +
  theme_minimal()

# Boxplot pour traveltime_val avant et après matching
# Densité
ggplot(combined_data_2021, aes(x = traveltime_val, fill = as.factor(treatment), color = as.factor(treatment))) +
  geom_density(alpha = 0.3) +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red")) +
  scale_color_manual(values = c("0" = "blue", "1" = "red")) +
  labs(title = "Densité de traveltime_val 2021", x = "traveltime_val", y = "Densité", fill = "Groupe", color = "Groupe") +
  theme_minimal()

# Boxplot
ggplot(combined_data_2021, aes(x = as.factor(treatment), y = traveltime_val, fill = as.factor(treatment))) +
  geom_boxplot() +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red")) +
  labs(title = "Comparaison de traveltime_val 2021", x = "Groupe", y = "traveltime_val", fill = "Groupe") +
  theme_minimal()

```

Nous avons visualisé la distribution des covariables entre les groupes de traitement et de contrôle à l'aide de courbes de densité et de boxplots. Avant le matching, on peut voir des écarts notables entre les distributions des deux groupes mettant en exergue un déséquilibre. Pour la variable treecover, la courbe de densité montre que les deux groupes ne se recouvrent pas exactement présentant ainsi un déséquilibre entre les groupes. Après matching, on observe une amélioration de la similarité entre les groupes, avec une superposition beaucoup plus importante des courbes de densité, et des médianes et interquartiles proches sur les boxplots.

## Test pour assouplir les calipers

Après le matching avec un caliper de 0.25, il se trouve que la taille des échantillons s'est rétrécie fortement. Nous avons 370 controles et 114 traitées avant le matching. Après le matching, seules 23 observations ont été matchées pour l'année 2008 et 42 pour l'année 2021. Nous allons augmenter la taille du caliper pour augmenter la taille des observations tout en vérifiant si SMD \< 0.1 .

```{r 2008}
# Matching avec caliper de 0.96 SD Mahalanobis 
matching_result_2008 <- matchit(
  treatment ~ treecover_area_val + slope_val + elevation_val + population_count_val + traveltime_val,
  data = matching_2008 %>% filter(groupe != "Exclu"),
  method = "genetic", 
  ...
  gen.match = gen_match_model,
  caliper = c(
    treecover_area_val = 0.96, 
    slope_val = 0.96,
    elevation_val = 0.96,
    population_count_val = 0.96,
    traveltime_val = 0.96
  )
)
