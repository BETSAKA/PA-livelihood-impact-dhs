...
```{r}
# Calcul des quintiles de l'indice de richesse (hv270)
hv271_2008 <- hv271_2008 %>%
  mutate(
    hv270_2008 = ntile(factor_score, 5)  # Divise hv270 en 5 quintiles égaux
  )

# Vérification de la répartition des quintiles
print(table(hv271_2008$hv270_2008))
print(summary(hv271_2008$hv270_2008))

# Vérification des valeurs manquantes
#if (any(is.na(hv271_2008$hv270_2008))) {
 # warning("Attention : Des valeurs manquantes sont présentes dans hv270_2008.")
#}

# Test de corrélation de Pearson et Spearman avec les scores de richesse DHS (hv271)
...
Les résultats des tests de corrélation se rapprochent et donc les quintiles de richesse reproduit bien la structure de de hv271.

```{r}
# Calcul des statistiques descriptives par quintile
stats_quintiles_2008 <- hh_2008_rec %>%
  group_by(hv270_2008) %>%
  summarise(
...
# Visualisation des quintiles de l'indice de richesse
ggplot(hh_2008_rec, aes(x = as.factor(hv270_2008), y = factor_score, fill = as.factor(hv270_2008))) +
...
# Histogramme des quintiles
ggplot(hh_2008_rec, aes(x = hv270_2008, fill = as.factor(hv270_2008))) +
  geom_bar(alpha = 0.8) +
  labs(
    title = "Répartition des observations par quintile de richesse",
    x = "Quintile de richesse",
...
# Densité des quintiles
ggplot(hh_2008_rec, aes(x = factor_score, fill = as.factor(hv270_2008), color = as.factor(hv270_2008))) +
  geom_density(alpha = 0.3, size = 1) +
  labs(
    title = "Densité des quintiles de l'indice de richesse",
    x = "Indice de richesse (factor_score)",
    y = "Densité"
  ) +
  scale_fill_brewer(palette = "Blues") +
...
# Statistiques descriptives par quintile 
stats_quintiles_2008 <- hh_2008_rec %>%
  group_by(hv270_2008) %>%
  summarise(
    mean_factor_score = mean(factor_score, na.rm = TRUE),
