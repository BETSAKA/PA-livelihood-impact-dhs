...
#Check that there is only one match per year
  if(length(shp_path)!=1 | length(hr_path) != 1) {
    stop(paste0("Erreur : ", length(shp_path), " fichier(s) .shp trouvés pour l'année", year, length(hr_path), " fichier(s) .dta trouvés pour l'année", year, " (attendu : 1 de chaque)"))
  }
#Read the data
...
#Return a list containing the two datasets 
