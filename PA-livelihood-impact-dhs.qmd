...
factor_vars_urban_2021 <- intersect(factor_vars_urban_2021, names(hh_urban_2021))

hh_urban_2021_rec <- hh_urban_2021 %>% select(all_of(factor_vars_urban_2021))
...
# Remove zero standard deviation variables
std_dev <- sapply(hh_urban_2021_rec, sd, na.rm = TRUE)

valid_cols <- names(std_dev[std_dev > 0 & !is.na(std_dev)])

hh_urban_2021_rec <- hh_urban_2021_rec %>%
  select(all_of(valid_cols))

# Calculate remain standard deviation 
std_dev <- sapply(hh_2021_rec, sd, na.rm = TRUE)

# Filter columns with a strictly positive standard deviation
valid_cols <- names(std_dev[std_dev > 0 & !is.na(std_dev)])
...
# PCA
pca_urban_2021 <- principal(hh_urban_2021_rec, ncp = 1, scale.unit = TRUE, graph = FALSE)

# Extracting factor score
hh_urban_2021 <- hh_urban_2021 %>%
  mutate(factor_score = pca_urban_2021$ind$coord[,1])

# Checking results 
summary(hh_urban_2021$factor_score)
...
library ("e1071") 
skewness(hv271_1997$wealth_index) #Asymétrie à gauche
