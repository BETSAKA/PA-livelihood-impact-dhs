...
hr_2008_rural <- hr_2008 %>%
  filter(hv025 == 2) %>%
  select(
    hhid, hv001, hv007, hv009, hv024, hv025, 
    hv201, hv202, hv204, hv205, hv206, hv207, 
    hv208, hv209, hv210, hv211, hv212, hv213, 
    hv214, hv215, hv219, hv220, hv221, hv225, 
    hv226, hv243a, hv243b, hv247, hv270, hv271
  )

# Jointure avec les données des ménages gps_cluster_2008 avec les caractéristiques géophysiques des ménages ruraux
merged_2008_rural <- gps_cluster_2008 %>%
  left_join(hr_2008_rural, by = c("DHSYEAR" = "hv007", "DHSCLUST" =  "hv001" ))

glimpse(merged_2008_rural)
summary(merged_2008_rural$hv270)

# Sélection des variables dans hr_2021 pour les zones rurales
hr_2021_rural <- hr_2021 %>%
  filter(hv025 == 2) %>%
  select(
    hhid, hv001, hv007, hv009, hv024, hv025, 
    hv201, hv202, hv204, hv205, hv206, hv207, 
...
# Jointure avec les données des ménages gps_cluster_2021 avec les caractéristiques géophysiques des ménages ruraux
merged_2021_rural <- gps_cluster_2021 %>%
  left_join(hr_2021_rural, by = c("DHSYEAR" = "hv007", "DHSCLUST" =  "hv001" ))

glimpse(merged_2021_rural)
summary(merged_2021_rural$hv270)
```
...
matching_2008 <- merged_2008_rural %>%
...
# Pré-traitement des données
matching_2021 <- merged_2008_rural %>%
  filter(groupe %in% c("Traitement", "Controle")) %>%
  mutate(
    treatment = ifelse(groupe == "Traitement", 1, 0),
    treecover_area_val = map_dbl(treecover_area, ~ {
...
bal_tab_pre <- bal.tab(
  treatment ~ treecover_area_val + slope_val + elevation_val + population_count_val + traveltime_val,
  data = matching_result %>% filter(groupe != "Exclu")
)
...
love.plot(bal_tab_pre, 
          ...
          title = "Equilibre des covariables avant Matching")


# Vérification de l'équilibre des covariables après matching
bal_tab_post <- bal.tab(matchit_result, un = TRUE)
...
love.plot(bal_tab_post,
          ...
          var.order = "unadjusted", 
          title = "Equilibre des covariables après Matching")

# Equilibre avant et après matching
summary(matchit_result, standardize = TRUE)
...
Avant le matching, il se trouve que plusieurs variables montrent un déséquilibre entre le groupe de traitement (185 groupes de traitement) et le groupe de contrôle (groupe de contrôle 690). Toutes les covariables présentent un SMD \> 0.1. Après le matching, l'équilibre entre les groupes pour les covariables est rétabli avec 81 unités traitées appariées avec 81 contrôles. Le matching
...
# Liste des covariables à visualiser
covariables <- c("treecover_area_val", "slope_val", "elevation_val", "population_count_val", "traveltime_val")

# Ajouter une colonne pour indiquer "Avant" ou "Après" le matching
matching_result$matching_status <- "Avant"
matched_data$matching_status <- "Après"

# Combiner les deux jeux de données
combined_data <- rbind(
  matching_result[, c("treecover_area_val", "slope_val", "elevation_val", "population_count_val", "traveltime_val", "treatment", "matching_status")],
  matched_data[, c("treecover_area_val", "slope_val", "elevation_val", "population_count_val", "traveltime_val", "treatment", "matching_status")]
)

#Treecover_area
ggplot(combined_data, aes(x = treecover_area_val, fill = as.factor(treatment))) +
  geom_histogram(alpha = 0.6, position = "identity", bins = 30) +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red"), labels = c("Contrôle", "Traitement")) +
  labs(title = "Distribution de treecover_area_val (avant et après matching)",
       x = "treecover_area_val", y = "Fréquence",
       fill = "Groupe") +
  theme_minimal()

#Slope
ggplot(combined_data, aes(x = slope_val, color = as.factor(treatment), fill = as.factor(treatment))) +
  geom_density(alpha = 0.3) +
  facet_wrap(~matching_status) +
  scale_color_manual(values = c("0" = "blue", "1" = "red"), labels = c("Contrôle", "Traitement")) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red"), labels = c("Contrôle", "Traitement")) +
  labs(title = "Distribution de slope_val (avant et après matching)",
       x = "slope_val", y = "Densité",
       color = "Groupe", fill = "Groupe") +
  theme_minimal()

# Elevation
ggplot(combined_data, aes(x = as.factor(treatment), y = elevation_val, fill = as.factor(treatment))) +
  geom_boxplot() +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red"), labels = c("Contrôle", "Traitement")) +
  labs(title = "Distribution de elevation_val (avant et après matching)",
       x = "Groupe", y = "elevation_val",
       fill = "Groupe") +
  theme_minimal()

# Population_count
ggplot(combined_data, aes(x = population_count_val, fill = as.factor(treatment), color = as.factor(treatment))) +
  geom_density(alpha = 0.3) +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red")) +
  scale_color_manual(values = c("0" = "blue", "1" = "red")) +
  labs(title = "Distribution de population_count_val", x = "population_count_val", y = "Densité", fill = "Groupe", color = "Groupe") +
  theme_minimal()

# Traveltime
ggplot(combined_data, aes(x = traveltime_val, fill = as.factor(treatment), color = as.factor(treatment))) +
  geom_density(alpha = 0.3) +
  facet_wrap(~matching_status) +
  scale_fill_manual(values = c("0" = "blue", "1" = "red")) +
  scale_color_manual(values = c("0" = "blue", "1" = "red")) +
  labs(title = "Distribution de traveltime_val ", x = "traveltime_val", y = "Densité", fill = "Groupe", color = "Groupe") +
  theme_minimal()


# Générer un Q-Q plot pour chaque covariable
par(mfrow = c(2, 3))  # Organise les plots en grille

for (var in covariables) {
  qqplot(
    matched_data[[var]][matched_data$treatment == 1],
    matched_data[[var]][matched_data$treatment == 0],
    xlab = paste("Traitement -", var),
    ylab = paste("Contrôle -", var),
...
Nous avons visualisé la distribution des covariables entre les groupes de traitement et de contrôle à l'aide de courbes de densité et de boxplots. Avant le matching, on peut voir des écarts notables entre les distributions des deux groupes mettant en exergue un déséquilibre. Par exemple pour la variable treecover, la courbe de densité montre que les deux groupes ne se recouvrent pas exactement présentant ainsi un déséquilibre entre les groupes. Après matching, on observe une amélioration de la similarité entre les groupes, avec une superposition beaucoup plus importante des courbes de densité, et des médianes et interquartiles proches sur les boxplots. Sur les Q-Q plots, on peut voir que les points sont alignés le long de la diagonale rouge indiquant ainsi que les distributions sont très similaires entre les groupes.

## Test pour assouplir les calipers

Après le matching avec un caliper de 0.25, il se trouve que la taille des échantillons s'est rétrécie fortement. Nous avons 690 controles et 185 traitées avant le matching. Après le matching, seules 81 observations ont été matchées. Nous allons augmenter la taille du caliper pour augmenter la taille des observations tout en vérifiant que SMD soit inférieur à 0.1 .

```{r }

# test sur différentes valeurs du caliper
calipers <- c(0.25, 0.3, 0.4)

results <- lapply(calipers, function(c) {
  matchit(
    treatment ~ treecover_area_val + slope_val + elevation_val + population_count_val + traveltime_val,
    data = matching_result %>% filter(groupe != "Exclu"),
    method = "genetic", 
    ...
    gen.match = gen_match_model,
    caliper = c(
      treecover_area_val = c, 
      slope_val = c,
      elevation_val = c,
      population_count_val = c,
      traveltime_val = c
    )
  )
})
names(results) <- paste0("caliper_", calipers)

# comparaison de l'équilibre et des tailles d'échantillon
library(cobalt)
...
for (name in names(results)) {
  ...
  match <- results[[name]]
  summary_stats <- summary(match, standardize = TRUE)
  
  print(summary_stats$nn)  
  print(summary_stats$sum.matched[, "Std. Mean Diff."])  # SMD
}

# Plot
# Caliper de 0.25
matched_data <- match.data(results[["caliper_0.25"]])

ggplot(matched_data, aes(sample = treecover_area_val, color = as.factor(treatment))) +
  stat_qq(distribution = qnorm) +
  stat_qq_line(distribution = qnorm) +
  ggtitle("Q-Q plot: treecover_area_val (caliper 0.25)") +
  theme_minimal()

# Caliper de 0.3
matched_data <- match.data(results[["caliper_0.3"]])

ggplot(matched_data, aes(sample = treecover_area_val, color = as.factor(treatment))) +
  stat_qq(distribution = qnorm) +
  stat_qq_line(distribution = qnorm) +
  ggtitle("Q-Q plot: treecover_area_val (caliper 0.3)") +
  theme_minimal()

# Caliper de 0.4
matched_data <- match.data(results[["caliper_0.4"]])

ggplot(matched_data, aes(sample = treecover_area_val, color = as.factor(treatment))) +
  stat_qq(distribution = qnorm) +
  stat_qq_line(distribution = qnorm) +
  ggtitle("Q-Q plot: treecover_area_val (caliper 0.4)") +
  theme_minimal()
```

Nous avons appliqué trois calipers différents pour notre analyse pour pouvoir comparer l'équilibre des covariables et la taille de l'échantillon apparié. Pour le caliper = 0.25, nous avons un excellent équilibre en terme de SMD mais on perd beaucoup d'observations car seules 81 paires sont appariées pour 690 contrôles et 185 traités. En augmentant le caliper à 0.3, le SMD reste excellent et plus de 34 paires ont été appariées par rapport à la caliper 0.25
