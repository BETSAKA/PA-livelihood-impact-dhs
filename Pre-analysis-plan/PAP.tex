...
from hunting accounted for 57~\% of household cash income in areas
...
tourism and inequality follows an inverted Kuznets curve dynamic in
developing countries, when tourism remains moderate, its growth reduces
binequalities, but when tourism becomes massive, further expansion
...
Madagascar, where we triangulated data with other sources (data from the
...
2023}) as relying on sources with errors that compromised the study's
conclusions.
...
\item
  ...
  protected areas (\citeproc{ref-joppa2010}{Joppa \& Pfaff, 2010}), as
...
imploemented in the SPEI R package
...
2024}). This only be performed if power is insufficient with two periods
of data.
