...
balance_after_table <- imap_dfr(bal_after, function(bt, year) {
  ...
  bt$Balance %>%
    ...
    transmute(
      ...
      Variable,
      SMD = Diff.Adj,
      Equilibre = if_else(abs(Diff.Adj) <= 0.1, "Équilibré", "Déséquilibré")
    )
})
...
write.csv(balance_after_table, "balance_after_table.csv", row.names = FALSE)
write.csv(balance_long, "balance_long.csv", row.names = FALSE)
