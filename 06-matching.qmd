...
data_matched_2011 <- readRDS("data/derived/data_matched_2011.rds") |> convert_labelled()
data_matched_2013 <- readRDS("data/derived/data_matched_2013.rds") |> convert_labelled()
data_matched_2016 <- readRDS("data/derived/data_matched_2016.rds") |> convert_labelled()
data_matched_2021 <- readRDS("data/derived/data_matched_2021.rds") |> convert_labelled()

# Uniformiser hv025 de 2013 avec ceux des autres 
data_matched_2013$hv025 <- "rural"

# Fusion des dataframes appariés 
df_all <- bind_rows(data_matched_1997, data_matched_2008, data_matched_2011, data_matched_2013, data_matched_2016, data_matched_2021)
...
st_drop_geometry(),
  gps_2013_initial %>%    
st_drop_geometry(),
  gps_2016_initial %>% st_drop_geometry(),
...
check_balance_before <- function(df, year, matching_vars, save_plot = FLASE, plot_dir = "plots"){
  data <- df %>%
    filter(GROUP %in% c("Treatment", "Control")) %>%
    mutate(treatment = if_else(GROUP == "Treatment", 1, 0))
  
  formula <- as.formula(
    paste("treatment ~", paste(matching_vars, collapse = "+"))
  )
  ...
  density_plot <- ggplot(data_long, aes(x = value, fill = GROUP)) +
