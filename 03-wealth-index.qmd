...
As our initial analysis lacked statistical power, we add MIS surveys 2016, 2013 and 2011 to the analysis. As mentionned in out PAP: 

> Additionally, the DHS program accompanied INSTAT surveys in 2011, 2013 and 2016, known as the Malaria Indicator Surveys (MIS). These surveys, which focus specifically on malaria related issues, do not include certain health or demographic questions that are included in the DHS surveys. However, all the variables relating to household living conditions that we will use in our study from the DHS surveys are also present in the MIS surveys. 

So hv271a (wealth index rural/urban) is also present in the survey household data. We recompute it for verification purposes.
...
We apply the same validation tests. First check that we obtain the same PCA loadings when performing the overall PCA.

```{r}
# Refit a national* PCA on the common set
vars_common_2016 <- common_pca_vars
hh_2016_nat <- hh_2016_rec %>%
  mutate(across(all_of(vars_common_2016), ~ replace_na(., 0)))

# Remove zero-variance variables nationally (rare but safe)
vars_common_2016 <- {
  zv <- hh_2016_nat %>%
    summarise(across(all_of(vars_common_2016), ~ var(., na.rm = TRUE))) %>%
    pivot_longer(everything(), names_to = "v", values_to = "var") %>%
    filter(var == 0) %>% pull(v)
  setdiff(vars_common_2016, zv)
}

pca_nat_2016 <- perform_pca(hh_2016_nat, vars_common_2016)



mis_loadings_path_2016 <- "data/raw/dhs/DHS_wealth_index/Madagascar MIS 2016.xlsx"
mis_2016_loadings <- read_excel(mis_loadings_path_2016, 
    sheet = "Common", skip = 3) %>%
  select(`Component Score Coefficient Matrix` = `...7`, Component = `1`) %>%
    mutate(variable = str_extract(`Component Score Coefficient Matrix`, "^[^ ]+"),
           dhs_factor = as.numeric(Component)) %>%
    select(variable, dhs_factor) %>% 
  filter(variable != "Extraction")

r_load <- tibble(variable = rownames(pca_nat_2016$loadings),
                 r_pca_loading = as.vector(pca_nat_2016$loadings[,1]))

comp_nat <- mis_2016_loadings %>% right_join(r_load, by = "variable")

ggplot(comp_nat, aes(dhs_factor, r_pca_loading)) +
  geom_point(alpha = .7) +
  geom_smooth(method = "lm", se = FALSE, linewidth = .2) +
  labs(title = "2016: MIS loadings vs reproduced (national)",
       x = "MIS factor loading", y = "Reproduced loading") +
  theme_minimal()

print(cor.test(comp_nat$dhs_factor, comp_nat$r_pca_loading, use = "complete.obs"))

```

Il semblerait qu'on ait un problème de comparaison : à investiguer.

```{r}
# Compare household scores to hv271
hh_2016_nat <- hh_2016_nat %>%
  mutate(score_nat = as.vector(scale(select(., all_of(vars_common_2016))) %*%
                                 pca_nat_2016$loadings[,1]))

ggplot(hh_2016_nat, aes(x = hv271, y = score_nat)) +
  geom_point(alpha = .4) +
  geom_smooth(method = "lm", se = FALSE, linewidth = .2) +
  labs(title = "2016: hv271 vs reproduced national score",
       x = "hv271 (DHS)", y = "Reproduced national factor score") +
  theme_minimal()

cor_nat_obs <- cor(hh_2016_nat$hv271, hh_2016_nat$score_nat,
                   method = "spearman", use = "complete.obs")
cor_nat_obs
```

Cela confirme que ce wealth index est à affiner.
On compare les scores ruraux et urbains : 

```{r}
# Compute domain scores with your existing PCAs (use your filtered var sets)
hh_2016_dom <- hh_2016_rec %>%
  mutate(
    domscore = case_when(
      hv025 == 1 ~ as.vector(scale(select(cur_data_all(), all_of(urban_vars_2016))) %*%
                               pca_urb_2016$loadings[,1]),
      hv025 == 2 ~ as.vector(scale(select(cur_data_all(), all_of(rural_vars_2016))) %*%
                               pca_rur_2016$loadings[,1])
    )
  )

# Scatter + correlations by domain vs hv271a
p_urb <- hh_2016_dom %>% filter(hv025 == 1)
p_rur <- hh_2016_dom %>% filter(hv025 == 2)

ggplot(p_urb, aes(domscore, hv271a)) +
  geom_point(alpha = .4) +
  geom_smooth(method = "lm", se = FALSE, linewidth = .2) +
  labs(title = "2016: domain score vs hv271a (Urban)",
       x = "Reproduced domain score", y = "hv271a (DHS)") +
  theme_minimal()

ggplot(p_rur, aes(domscore, hv271a)) +
  geom_point(alpha = .4) +
  geom_smooth(method = "lm", se = FALSE, linewidth = .2) +
  labs(title = "2016: domain score vs hv271a (Rural)",
       x = "Reproduced domain score", y = "hv271a (DHS)") +
  theme_minimal()

spearman_urb_2016 <- cor(p_urb$hv271a, p_urb$domscore, method = "spearman", use = "complete.obs")
spearman_rur_2016 <- cor(p_rur$hv271a, p_rur$domscore, method = "spearman", use = "complete.obs")

tibble(
  domain = c("Urban","Rural"),
  spearman_dom_vs_hv271a = c(spearman_urb_2016, spearman_rur_2016)
)

```

## Calculating MIS 2011 and 2013

On calcule pour 2011 et 2013.
...
DHS n'a pas publié de table avec les loadings pour les MIS 2011 et 2013. On compare toutefois avec les valeurs hv271 contenues dans les données.

```{r}
# Helper: national validation given a recoded hh file and the variable list
validate_nat <- function(hh_rec, vars, year_label = "2011") {
  # keep available vars and drop zero-variance nationally
  vars_nat <- remove_zero_var(hh_rec, vars)
  stopifnot(length(vars_nat) >= 3)

  # national PCA
  pca_nat <- perform_pca(hh_rec, vars_nat)

  # compute reproduced national score
  hh_nat <- hh_rec %>%
    mutate(
      score_nat = as.vector(scale(dplyr::select(., all_of(vars_nat))) %*%
                              pca_nat$loadings[, 1])
    )

  # scatter + Spearman vs hv271
  print(
    ggplot(hh_nat, aes(x = hv271, y = score_nat, color = as.factor(hv025))) +
      geom_point(alpha = 0.4) +
      geom_smooth(method = "lm", se = FALSE, linewidth = 0.2) +
      labs(title = paste0(year_label, ": hv271 vs reproduced national score"),
           x = "hv271 (DHS)", y = "Reproduced national score") +
      theme_minimal() +
      guides(color = "none")
  )

  rho <- cor(hh_nat$hv271, hh_nat$score_nat, method = "spearman", use = "complete.obs")
  data.frame(year = year_label, spearman_hv271_vs_nat = rho)
}

# 2011 national validation
val_2011 <- validate_nat(hh_2011_rec, vars_recodeVI, "2011")
val_2011

# 2013 national validation
val_2013 <- validate_nat(hh_2013_rec, vars_recodeVI, "2013")
val_2013
```

OK, là aussi il faudra affiner, c'est encore trop approximatif.
...
gps_2011 <- st_read("data/raw/dhs/DHS_2011/MDGE61FL/MDGE61FL.shp")
gps_2013 <- st_read("data/raw/dhs/DHS_2013/MDGE6AFL/MDGE6AFL.shp")
gps_2016 <- st_read("data/raw/dhs/DHS_2016/MDGE71FL/MDGE71FL.shp")
...
clusters_2011 <- prepare_clusters(hh_2011_rural, gps_2011, 2011)
clusters_2013 <- prepare_clusters(hh_2013_rural, gps_2013, 2013)
clusters_2016 <- prepare_clusters(hh_2016_rural, gps_2016, 2016)
...
value_range <- range(
  ...
  clusters_2011$wealth_centile_weighted,
  clusters_2013$wealth_centile_weighted,
  clusters_2016$wealth_centile_weighted,
...
tm3 <- make_map(clusters_2011, 2011, value_range)
tm4 <- make_map(clusters_2013, 2013, value_range)
tm5 <- make_map(clusters_2016, 2016, value_range)
tm6 <- make_map(clusters_2021, 2021, value_range)

# Mode interactif
tmap_mode("view")

# Affichage côte à côte
tmap_arrange(tm1, tm2, tm3, tm4, tm5, tm6, ncol = 3)
```
...
hh_2011_rural <- add_zscore_from_centile(hh_2011_rural, "wealth_centile_rural_weighted", "hv001")
hh_2013_rural <- add_zscore_from_centile(hh_2013_rural, "wealth_centile_rural_weighted", "hv001")
hh_2016_rural <- add_zscore_from_centile(hh_2016_rural, "wealth_centile_rural_weighted", "hv001")
```
...
clusters_2011_zscore <- prepare_clusters_z(hh_2011_rural, gps_2011, 2011)
clusters_2013_zscore <- prepare_clusters_z(hh_2013_rural, gps_2013, 2013)
clusters_2016_zscore <- prepare_clusters_z(hh_2016_rural, gps_2016, 2016)
...
z_range <- range(
  ...
  clusters_2011_zscore$wealth_zscore,
  clusters_2013_zscore$wealth_zscore,
  clusters_2016_zscore$wealth_zscore,
...
tm3 <- make_map_z(clusters_2011_zscore, 2011, z_range)
tm4 <- make_map_z(clusters_2013_zscore, 2013, z_range)
tm5 <- make_map_z(clusters_2016_zscore, 2016, z_range)
tm6 <- make_map_z(clusters_2021_zscore, 2021, z_range)
