...
# Crée une fonction pour obtenir les seuils de centile pondérés
get_centile_thresholds <- function(x, w, probs = seq(0, 1, by = 0.01)) {
  Hmisc::wtd.quantile(x, weights = w, probs = probs, na.rm = TRUE)
}

# Percentile 2021-----------------------------------------------------
thresholds <- get_centile_thresholds(hh_2021_rural$wealth_index_rural, hh_2021_rural$w)

# Attribue à chaque observation son centile pondéré
hh_2021_rural<- hh_2021_rural %>%
  mutate(
    wealth_centile_rural = cut(
      wealth_index_rural,
      breaks = c(-Inf, thresholds[-1]), 
      labels = 1:100,
      include.lowest = TRUE,
      right = TRUE
    ) %>% as.integer()
...
# Percentile 2008----------------------------------------------------------
thresholds <- get_centile_thresholds(hh_2008_rural$wealth_index_rural, hh_2008_rural$w)

# Attribue à chaque observation son centile pondéré
hh_2008_rural<- hh_2008_rural %>%
  mutate(
    wealth_centile_rural = cut(
      wealth_index_rural,
      breaks = c(-Inf, thresholds[-1]),  # évite double inclusion des bornes
      labels = 1:100,
      include.lowest = TRUE,
      right = TRUE
    ) %>% as.integer()
...
# Percentile 1997-------------------------------------------------
thresholds <- get_centile_thresholds(hh_1997_rural$wealth_index_rural, hh_1997_rural$w)

# Attribue à chaque observation son centile pondéré
hh_1997_rural<- hh_1997_rural %>%
  mutate(
    wealth_centile_rural = cut(
      wealth_index_rural,
      breaks = c(-Inf, thresholds[-1]),  # évite double inclusion des bornes
...
hh_2021_rural <- hh_2021_rural %>%
  group_by(hv001) %>%
  mutate(
   mean_wealth = mean(wealth_centile_rural, na.rm = TRUE),
   sd_wealth = sd(wealth_centile_rural, na.rm = TRUE),
   z_score_wealth = (wealth_centile_rural - mean_wealth) / sd_wealth) %>% 
  ungroup () %>%
  select(-mean_wealth, -sd_wealth)


# Save for later use
write_rds(hh_2021_rec, "data/derived/hh_2021_rec.rds")
