...
spearman_urban <- cor(
  ...
  hh_2021_rec %>% filter(hv025 == 1) %>% pull(wealth_index_rural),
...
hh_1997_pca_scores <- hh_1997_rec %>%
  select(hv001, hv002, hv005, hv025, hv271, wealth_index_rural)
...
hh_2021_rec <- hh_2021_rec %>%
  mutate(w = hv005 / 1e6) %>%
  group_by(hv025)%>%
  arrange(wealth_index_rural) %>%
  mutate(
    cum_w = cumsum(w),
    total_w = sum(w),
    pct_w = cum_w / total_w,
    wealth_centil_rural= floor(pct_w * 100) + 1
  ) %>%
  ungroup() %>%
  select(-cum_w, -total_w, -pct_w, -w)

# Percentiles 2008
hh_2008_hv271a <- hh_2008_hv271a %>%
  mutate(w = hv005 / 1e6) %>%
  group_by(hv025) %>%
  arrange(wealth_index_rural) %>%
  mutate(
    cum_w = cumsum(w),
    total_w = sum(w),
    wealth_centile_rural = floor(cum_w / total_w * 100) + 1 
  ) %>%
  ungroup () %>%
  select(-cum_w, -total_w)

# Save for later use
write_rds(hh_2008_hv271a, "data/derived/hh_2008_hv271a.rds")

# Percentiles 1997
hh_1997_pca_scores <- hh_1997_pca_scores %>%
  mutate(w = hv005/1e6) %>%
  filter(hv025 == 2 & !is.na(wealth_index_rural)) %>%
  arrange(wealth_index_rural) %>%
  mutate(
    cum_w = cumsum(w),
    total_w = sum(w),
...
hh_2021_rec <- hh_2021_rec %>%
  group_by(hv001) %>%
  mutate(
   mean_wealth = mean(wealth_centil_rural, na.rm = TRUE),
   sd_wealth = sd(wealth_centil_rural, na.rm = TRUE),
   z_score_wealth = (wealth_centil_rural - mean_wealth) / sd_wealth) %>% 
  ungroup ()
