...
did_fit_wi <- function(d, years){
  ...
  list(model = m, vcov = vc, data=dd, tidy=out)
}

# Estimation
res_placebo_wi <- did_fit_wi(data_all_mod, c(1997, 2008))
res_main_wi <- did_fit_wi(data_all_mod, c(2008, 2021))

m_placebo_wi <- res_placebo$model
m_main_wi <- res_main$model

se_placebo_wi <- sqrt(diag(res_placebo_wi$vcov))

se_main_wi <- sqrt(diag(res_main_wi$vcov))


# Tableau de l'estimation DID avec erreurs standards par cluster 
stargazer(m_placebo_wi, m_main_wi,
          type = "text",
...
# Estimation
res_placebo_zs <- did_fit_zs(data_all_mod, c(1997, 2008))
res_main_zs <- did_fit_zs(data_all_mod, c(2008, 2021))

m_placebo_zs <- res_placebo$model
m_main_zs <- res_main$model

se_placebo_zs <- sqrt(diag(res_placebo_zs$vcov))

se_main_zs <- sqrt(diag(res_main_zs$vcov))


# Tableau de l'estimation DID avec erreurs standards par cluster 
stargazer(m_placebo_zs, m_main_zs,
          type = "text",
          se = list(se_placebo_zs, se_main_zs),
          ci = FALSE,
          title = "Estimation DID avec erreurs standards robustes cluster",
          dep.var.labels = "zscore_wealth",
          covariate.labels = c("Treatment x Post"),
          star.cutoffs = c(0.05, 0.01, 0.001))
