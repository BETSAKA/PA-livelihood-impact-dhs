...
## Definition of treatment periods // TITRE A REVOIR

La période de pré-traitement sera la période de 1997 à 2008. La période de post-traitement la période 2008 et 2021. Le groupe de traitement ce sont les ménages vivant à moins de 10km des aires protégées créées entre 2008 et 2021. Le groupe de contrôle ce sont les ménages vivant à plus de 10km des aires protégées crées jusqu'en 2021.
Les ménages vivant à moins de 10km d'une aire protégée créée avant 2008 seront exclus de l'analyse.
L'estimation d'impact consiste à comparer l'évolution du niveau de vie du groupe de traitement et du groupe de contrôle entre 2008 et 2021. Si une différence significative apparaît, on considérera que la création des aires protégées a eu un impact sur les conditions de vie des riverains, sous réserve que le test placebo soit concluant.
Le test placebo consiste à comparer l'évolution du niveau de vie du même groupe de traitement et du même groupe de contrôle entre 1997 et 2008. Si les conditions de vie évoluent parallèlement dans ces deux groupes, alors on considérera le test placebo comme concluant.
...
# Renommer les noms des spei du dataframe de 2008 et 2021
data_matched_1997 <- data_matched_1997 %>%
  rename(
...
did_placebo_wi <- lm(wealth_centile_rural_simple ~ placebo_treated + post + 
                       placebo_treated:post + spei_wc_n_2 +  spei_wc_n_1 + 
                       spei_wc_n + hv219 + hv220, data = placebo_data)
...
# Statistiques
evolution_placebo_wi <- placebo_data %>%
...
# Plot
ggplot(evolution_placebo_wi, 
       aes(x = DHSYEAR, y = mean_value, color = GROUP, group = GROUP)) +
  geom_point(size = 4) + 
  geom_line(size = 1.2) +
  geom_errorbar(aes(ymin = mean_value - 1.96 * se, ymax = mean_value + 1.96 * se),
                width = 0.2, size = 0.8, alpha = 0.6) +
  geom_vline(xintercept = 2008, linetype = "dashed", color = "grey", size = 1) + 
  annotate("text", x = 2007.5, y = max(evolution_placebo_wi$mean_value) * 0.95,
...
```{r}
#--------------------------------------------------
# Test Placebo sur 1997-2008 with outcome = z-score
#--------------------------------------------------

# Estimation
did_placebo_zs <- lm(zscore_wealth ~ placebo_treated + post + placebo_treated:post + 
                    spei_wc_n_2 +  spei_wc_n_1 + spei_wc_n + hv219 + hv220, 
                  data = placebo_data)
summary(did_placebo_zs)

# Coefficient presentation
tidy(did_placebo_zs)

# Statistiques
evolution_placebo_zs <- placebo_data %>%
  group_by(DHSYEAR, GROUP) %>%
  summarise(
...
# Ajout d'un label pour différencier la période pré/ post
evolution_placebo_zs <- evolution_placebo_zs %>%
  mutate(period = ifelse(DHSYEAR < 2008, "Pre-treatment", "Post-treatment"))

# Plot
ggplot(evolution_placebo_zs,
       aes(x = DHSYEAR, y = mean_value, color = GROUP, group = GROUP)) +
  geom_point(size = 4) +
  geom_line(size = 1.2) +
  geom_errorbar(aes(ymin = mean_value - 1.96 * se, ymax = mean_value + 1.96 * se),
                width = 0.2, size = 0.8, alpha = 0.6) +
  geom_vline(xintercept = 2008, linetype = "dashed", color = "grey", size = 1) + annotate("text", x = 2007.5, y = max(evolution_placebo_zs$mean_value) * 0.95,
    label = "Traitement", angle = 90, vjust = -0.5, size = 4, color = "grey30") +
  scale_color_manual(
    values = c("Treatment" = "#D55E00", "Control" = "#0072B2")) +
  scale_x_continuous(breaks = c(1997, 2008), labels = c("1997", "2008")) +
...
did_2008_2021 <- lm(
  wealth_centile_rural_simple  ~ treated * post + sex_head + age_head + 
    ...
    `spei_wc_n_2` + `spei_wc_n_1` + `spei_wc_n`, 
  data = model_data )
...
```{r}
#---------------------------------------------
# DID Estimation with controls for z score (2008-2021)
#--------------------------------------------
# Treatment period: 2008-2021
