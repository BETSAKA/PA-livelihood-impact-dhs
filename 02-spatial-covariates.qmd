...
Avant chargement des données des clusters GPS, nous définissons les systèmes de coordonnées. Par défaut, les coordonnées GPS sont fournies dans le système de référence géographique WGS 84 (EPSG: 4326). Toutefois, pour effectuer des calculs précis de distances ou de surfaces, nous adoptons le système de projection officiel de Madagascar, qui est le Laborde (EPSG: 29702). On travaille en 4326 puis temporairement reprojetées en 29702 lorsque les opérations métriques sont nécessaires. Après le chargement des données DHS, certains clusters n'ont pas de coordonnées valides (0,0). on les retire pour éviter des erreurs dans les calculs géospatiaux.
...
>>>>>>> 21ccef7e3da7335698ccf91bfadc9b123155d500
...
On définit notre environnement de travail pour automatiser le téléchargement et le stockage des données spatiales à l'aide de mapme.biodiversity, issus de plusieurs sources mondiaux. Nous utilisons la fonction get_resources () pour cela en démarrant un chronomètre avec le package tictoc pour mesurer le temps d'exécution du bloc de code.
...
if (file.exists("data/derived/variable_all_cluster.rds")) {
  variable_all_cluster <- readRDS("data/derived/variable_all_cluster.rds")
} else {
...
if (file.exists("data/derived/buffer_all_spei.rds")) {
  buffer_all_spei <- read_rds("data/derived/buffer_all_spei.rds")
} else {
  buffer_all_spei <- buffer_all_spei %>% 
    mutate(
      lat = st_coordinates (st_centroid(geometry))[,2])
  
  # Extract the first cluster's climate time series
  row1 <- buffer_all_spei [1,]
 ...
 write_rds(x = buffer_all_spei, file = "data/derived/buffer_all_spei.rds")
```
