...
Ce document vise à charger les données géophysiques des ménages à l'aide du package R mapme.biodiversity (<https://mapme-initiative.github.io/mapme.biodiversity/index.html>) @Gorgen2022. Ce package va automatiser la récupération de données volumineuses au format raster et leur traitement pour produire une série d'indicateurs appliqués à des polygones.
...
- Les données worldclim @VicenteSerrano2010
...
if (!file.exists("data/derived/spatial_covars_staggered.rds")) {
 ...
 # if (!file.exists("data/derived/step1_tmax.rds")) {
 # tic()
 # with_progress({
 #  buffer_all <- buffer_all %>%
   #   calc_indicators(calc_temperature_max_wc(engine = "extract", stats = "mean"))
  #})
  #toc()
  #write_rds(buffer_all, "data/derived/step1_tmax.rds")
#} else {
#  buffer_all <- read_rds("data/derived/step1_tmax.rds")
#} # 16451.43 sec elapsed 
 ...
 # if (!file.exists("data/derived/step2_tmin.rds")) {
 # tic()
 # with_progress({
 #   buffer_all <- buffer_all %>%
 #     calc_indicators(calc_temperature_min_wc(engine = "extract", stats = "mean"))
 # })
 # toc()
 # write_rds(buffer_all, "data/derived/step2_tmin.rds")
#} else {
#  buffer_all <- read_rds("data/derived/step2_tmin.rds")
#} # 17 177,79 sec elapsed
 ...
 # if (!file.exists("data/derived/step3_precip.rds")) {
 # tic()
 # with_progress({
  #  buffer_all <- buffer_all %>%
  ...
  #})
 # toc()
 # write_rds(buffer_all, "data/derived/step3_precip.rds")
#} else {
#  buffer_all <- read_rds("data/derived/step3_precip.rds")
#} # 18397.88 sec elapsed 
 ...
 # if (!file.exists("data/derived/step4_treecover.rds")) {
 # tic()
 # with_progress({
 #   buffer_all <- buffer_all %>%
 #     calc_indicators(calc_treecover_area(years = 2000, min_size = 1, min_cover = 10))
 # })
  #toc()
 # write_rds(buffer_all, "data/derived/step4_treecover.rds")
#} else {
 # buffer_all <- read_rds("data/derived/step4_treecover.rds")
#} # 1848 sec elapsed
 ...
 # if (!file.exists("data/derived/step5_slope.rds")) {
 # tic()
 # with_progress({
 #   buffer_all <- buffer_all %>%
 #     calc_indicators(calc_slope(engine = "extract", stats = "mean"))
 # })
 # toc()
 # write_rds(buffer_all, "data/derived/step5_slope.rds")
#} else {
#  buffer_all <- read_rds("data/derived/step5_slope.rds")
#} # 1316.38 sec elapsed 
...
#  if (!file.exists("data/derived/step6_elevation.rds")) {
#  tic()
#  with_progress({
#    buffer_all <- buffer_all %>%
#      calc_indicators(calc_elevation(engine = "extract", stats = "mean"))
 # })
 # toc()
#  write_rds(buffer_all, "data/derived/step6_elevation.rds")
#} else {
#  buffer_all <- read_rds("data/derived/step6_elevation.rds")
#} # 1122.25 sec elapsed 
  ...
  toc() # 5233,48 sec elapsed | # 224.2 sec elapsed 
  
  
 # if (!file.exists("data/derived/step7_population.rds")) {
 # tic()
#  with_progress({
#    buffer_all <- buffer_all %>%
#      calc_indicators(calc_population_count(engine = "extract", stats = "mean"))
#  })
#  toc()
#  write_rds(buffer_all, "data/derived/step7_population.rds")
#} else {
#  buffer_all <- read_rds("data/derived/step7_population.rds")
#} 
...
#  if (!file.exists("data/derived/step8_accessibility.rds")) {
#  tic()
#  with_progress({
#    buffer_all <- buffer_all %>%
#      calc_indicators(calc_traveltime_2000(engine = "extract", stats = "mean"))
#  })
 # toc()
#  write_rds(buffer_all, "data/derived/step8_accessibility.rds")
#} else {
#  buffer_all <- read_rds("data/derived/step8_accessibility.rds")
#} # 464.14 sec elapsed 
